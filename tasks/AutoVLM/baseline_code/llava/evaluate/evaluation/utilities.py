...
import openai  # Assuming the OpenAI package is installed


def get_chat_response(prompt, temperature=0, max_tokens=256, n=1, patience=10000000, sleep_time=10):
    messages = [
    ...
    while patience > 0:
        ...
        try:
            ...
            # Process the response for a single completion
            if n == 1:
                prediction = response.choices[0].message.content.strip()
                if prediction:
                    return prediction
            else:
                # Process the response for multiple completions
