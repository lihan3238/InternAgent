...
from internagent.experiment_store import ExperimentStore
...
class IdeaGenerator:
    ...
    def __init__(self, args, logger, store=None, history_context: str = ""):
        self.args = args
        self.logger = logger
    ...
    async def generate_ideas(self):
                ...
                if self.status == "awaiting_feedback":#idea需要人机交互
                    ...
                    try:
                        ...
                        def prompt_for_feedback():
                            ...
                            lines = []
                            deadline = time.time() + 20  # auto-continue after 20s with no input
                            while True:
                                remaining = deadline - time.time()
                                if remaining <= 0:
                                    print("No input within 20s; continuing automatically.")
                                    break
                                try:
        ...
        augmented = self._attach_similar_experiments(top_ideas)
        return augmented, session_json

    def _attach_similar_experiments(self, ideas):
        """Attach similar experiment context for RAG/dedup into idea payloads."""
        if not self.store:
            return ideas
        cfg = getattr(self.interface, "config", {}) if hasattr(self, "interface") else {}
        store_cfg = cfg.get("store", {}) if isinstance(cfg, dict) else {}
        top_k = store_cfg.get("similar_top_k", 3)

        def _extract_text(idea):
            details = (
            ...
            raw_parts = [
                ...
                details.get("method"),
                details.get("baseline_summary"),
                idea.get("background", "") if isinstance(idea, dict) else "",
            ]
            text_parts = []
        ...
        for idea in ideas or []:
            ...
            similar = self.store.search_similar_motivation(text, top_k=top_k)
            idea["similar_experiments"] = similar
        return ideas
...
class ExperimentRunner:
    ...
    def __init__(self, args, logger, config=None, store=None):
        self.args = args
        self.logger = logger
    ...
    def _persist_experiment(self, idea_info, idea, status, artifact_dir=None, run_history=None, error=None, summary=None):
        """Persist experiment metadata after execution."""
        if not self.store:
            return None
        rel_artifact_dir = None
        if artifact_dir:
            try:
                rel_artifact_dir = osp.relpath(artifact_dir, start=self.args.output_dir)
            except Exception:
                rel_artifact_dir = artifact_dir

        record = {
            "idea_id": idea.get("id") if isinstance(idea, dict) else None,
            "idea_name": idea_info.get("name"),
            "title": idea_info.get("title"),
            "motivation": idea_info.get("description"),
            "background": idea.get("background", "") if isinstance(idea, dict) else "",
            "design_insight": idea_info.get("method"),
            "constraints": idea.get("constraints", []) if isinstance(idea, dict) else [],
            "similar_experiments": idea.get("similar_experiments") if isinstance(idea, dict) else [],
            "status": status,
            "error": error,
            "artifacts": {"artifact_dir": rel_artifact_dir} if rel_artifact_dir else {},
            "run_history": run_history,
            "summary": summary,
        }
        try:
            return self.store.add_experiment(record)
        except Exception as exc:
            self.logger.warning(f"Failed to persist experiment record: {exc}")
            return None
    ...
    def run_aider_experiment(self, base_dir, results_dir, idea):
        ...
        try:
            ...
            success = perform_experiments_aider(idea, folder_name, experiment_model, baseline_results)
    ...
    def run_experiments(self, base_dir, results_dir, ideas):
        ...
        for idx, idea in enumerate(ideas, 1):
            ...
            try:
                ...
                if exp_id:
                    result_entry['experiment_id'] = exp_id
