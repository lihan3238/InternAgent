...
def _normalize_line_endings(file_path):
    """Replace CRLF/CR with LF to avoid shell path issues."""
    try:
        ...
        print(f"[WARN] Failed to normalize line endings for {file_path}: {exc}")
...
def info_traceback(stderr):
    ...
    # Keep only the most recent stack frames to avoid prompt bloat
    matches = matches[-5:]
    if len(message) > 1000:
...
def run_experiment(folder_name, run_num, timeout=27000):
    ...
    # Normalize line endings to avoid \r-related issues in scripts/paths
    _normalize_line_endings(osp.join(cwd, "launcher.sh"))
    _normalize_line_endings(experiment_dst)
    plot_src = osp.join(cwd, "plot.py")
    _normalize_line_endings(plot_src)

    # Copy plot.py only if it exists in the experiment folder
...
# PERFORM EXPERIMENTS
def perform_experiments(idea, folder_name, model_name, baseline_results) -> bool:
    """Run aider-driven experiment loop while resetting coder context each iteration."""
    ...
    while run < MAX_RUNS + 1:
        ...
        # Refresh file list each iteration so newly created files are visible to coder
        fnames = [
            osp.join(folder_name, "experiment.py"),
            osp.join(folder_name, "notes.txt"),
        ]
        plot_candidate = osp.join(folder_name, "plot.py")
        if osp.exists(plot_candidate):
            fnames.append(plot_candidate)

        # Recreate coder each iteration to avoid unbounded chat history
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                msg = str(exc)
                if ("RateLimit" in msg) or ("rate limit" in msg.lower()) or ("TPM limit" in msg):
                    wait = min(5 * (2 ** (attempt - 1)), 60)
                    print(f"[WARN] Rate limit hit (attempt {attempt}/{max_attempts}); sleeping {wait}s before retry...")
                    time.sleep(wait)
                    continue
                raise
        print(coder_out)
        if "litellm.BadRequestError" in coder_out:
            print("Context limit reached, aborting this run.")
            return False
        ...
        return_code, next_prompt_content, traceback, message = run_experiment(folder_name, run)

        if return_code == 0:
            run += 1
            current_iter = 0
            next_prompt = next_prompt_content
            continue
        ...
        if traceback:
            functions_codes = ""
            for t in traceback:
                functions_codes += f"line: {t[1]}, function: {t[2]}, codes: {t[3]} \n"

            next_prompt = DEBUG_PROMPT_WITH_STRUCTURE.format(
                error_messages=next_prompt_content,
                code_structure=functions_codes,
            )
        else:
            next_prompt = (
                f"Experiment failed with non-traceback error (return code {return_code}).\n"
                f"Output tail:\n{next_prompt_content}\n\n"
                "Please fix the code. If changing args, ensure updated defaults."
            )
