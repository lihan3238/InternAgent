...
**CRITICAL INSTRUCTION: PREVENT TOKEN LIMIT ERRORS**
The proposed method likely requires significant changes to multiple components (Model, Dataset, Training Loop).
To avoid hitting the output token limit, **DO NOT implement everything in a single response.**

Please follow this strictly incremental process:

1. **Plan Phase**: First, output your experiment plan and the sequence of code changes you intend to make.
2. **Implementation Phase 1 (Model Only)**: Implement ONLY the new model classes (e.g., new Layers, Embeddings) in `experiment.py`. Do not touch data loading yet.
3. **Implementation Phase 2 (Data/Utils)**: Once the model classes are in place, update the `Dataset` class and data loading functions to support the new features.
4. **Implementation Phase 3 (Integration)**: Finally, update the training loop and `main` function to connect the new data to the new model.

**After you complete a phase, pause and let the system process your changes before moving to the next.**

Note that we already provide the vanilla baseline results:
{baseline_results}

After you complete your changes for a run, we will execute `bash launcher.sh run_i`.

**Important**:
- Modify `experiment.py` for logic and `plot.py` for visualization.
- Any modifications to `argparse` must be set as `default` values to ensure they run automatically.
- **If you feel your response is getting too long, stop and ask to continue in the next turn.**
"""
