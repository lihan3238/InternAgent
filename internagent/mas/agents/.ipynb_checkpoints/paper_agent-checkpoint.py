...
"""
    ...
    def _resolve_iteration_result_dirs(self, root_output_dir: str, params: Dict[str, Any]) -> List[str]:
        ...
        for entry in raw_dirs or []:
            ...
            if not os.path.isabs(candidate):
                joined = os.path.join(root_output_dir, entry)
                candidate = joined if os.path.exists(joined) else os.path.abspath(entry)
    ...
    async def execute(self, context: Dict[str, Any], params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        root_output_dir = params.get("root_output_dir") or self.config.get("output_dir") or os.getcwd()
...
if __name__ == "__main__":
    # Simple test harness for local debugging of PaperGenerationAgent.
    # Usage: python paper_generation_agent.py [path_to_iteration_folder]
    import asyncio
    import sys

    logging.basicConfig(level=logging.INFO)

    # Default to the attached iteration_1 results path inside the repo
    default_iteration = os.path.join(os.getcwd(), "results", "AutoForecast", "iteration_1")
    iteration_path = sys.argv[1] if len(sys.argv) > 1 else default_iteration

    print(f"[paper_generation_agent.py] Running test against: {iteration_path}")

    # Load ideas.json if present
    ideas_file = os.path.join(iteration_path, "ideas.json")
    ideas = []
    if os.path.exists(ideas_file):
        try:
            ...
            print(f"Failed to load ideas.json: {e}")

    # Collect simple run-level results from subfolders
    results_list = []
    try:
        for child in os.listdir(iteration_path):
            child_path = os.path.join(iteration_path, child)
            if not os.path.isdir(child_path):
                continue
            # Look for run_* directories inside experiment folders
            if child.startswith('run_'):
                # top-level run (rare) — include
                run_dir = child_path
                final_info = os.path.join(run_dir, 'final_info.json')
                entry = {'artifact_dir': run_dir}
                if os.path.exists(final_info):
                    try:
                        entry['final_info'] = json.load(open(final_info, 'r', encoding='utf-8'))
                    except Exception:
                        entry['final_info'] = None
                results_list.append(entry)
            else:
                # experiment folder (timestamped)
                try:
                    for sub in os.listdir(child_path):
                        subp = os.path.join(child_path, sub)
                        if os.path.isdir(subp) and sub.startswith('run_'):
                            run_dir = subp
                            final_info = os.path.join(run_dir, 'final_info.json')
                            entry = {'artifact_dir': run_dir}
                            if os.path.exists(final_info):
        ...
        print(f"Failed to scan iteration folder: {e}")

    context = {
        'task': {'name': 'AutoForecast Iteration Test', 'description': f'Paper generation test for {iteration_path}'},
        'ideas': ideas,
        'results': results_list,
    }

    detected_idea_dirs = _discover_idea_result_dirs(iteration_path)

    # Try to load environment variables from a .env file in the repository root.
    # This helps tests pick up keys like OPENAI_API_KEY when run locally.
    env_path = os.path.join(os.getcwd(), '.env')
    if os.path.exists(env_path):
        try:
            from dotenv import load_dotenv
            load_dotenv(env_path, override=False)
            print(f"[paper_generation_agent.py] Loaded environment variables from {env_path}")
        except Exception:
            # Fallback: manual parse of simple KEY=VALUE lines (no expansion)
            try:
                with open(env_path, 'r', encoding='utf-8') as ef:
                    for ln in ef:
                        ...
                        if '=' in ln:
                            ...
                            # Only set if not already in environment to avoid overwriting
                            if k not in os.environ:
                                os.environ[k] = v
                print(f"[paper_generation_agent.py] Loaded .env manually from {env_path}")
            except Exception as e:
                print(f"[paper_generation_agent.py] Failed to load .env: {e}")

    # Attempt to create real models from config/default_config.yaml; fall back to DummyModel
    model = None
    small_model = None
    global_conf = {}
    paper_conf = {}
    ...
    if os.path.exists(default_conf_path):
        try:
            import yaml
            with open(default_conf_path, 'r', encoding='utf-8') as cf:
                global_conf = yaml.safe_load(cf) or {}

            paper_conf = (global_conf.get('agents') or {}).get('paper_generation', {}) or {}
            try:
                from internagent.mas.models.model_factory import ModelFactory

                big_cfg = paper_conf.get('big_model') or {}
                if big_cfg:
                    cfg_for_big = dict(global_conf)
                    cfg_for_big.update(big_cfg)
                    try:
                        big_model = ModelFactory.create_model(cfg_for_big)
                        model = big_model
                        print('[paper_generation_agent.py] Created big_model from default_config.yaml')
                    except Exception as e:
                        print(f'[paper_generation_agent.py] Failed to create big_model: {e}')

                small_cfg = paper_conf.get('small_model') or {}
                if small_cfg:
                    cfg_for_small = dict(global_conf)
                    cfg_for_small.update(small_cfg)
                    try:
                        small_model = ModelFactory.create_model(cfg_for_small)
                        print('[paper_generation_agent.py] Created small_model from default_config.yaml')
                    except Exception as e:
                        print(f'[paper_generation_agent.py] Failed to create small_model: {e}')
            except Exception as e:
                print(f'[paper_generation_agent.py] ModelFactory unavailable or failed: {e}')
        except Exception as e:
            print(f'[paper_generation_agent.py] Failed to load default_config.yaml: {e}')
    else:
        print(f'[paper_generation_agent.py] No default_config.yaml found at {default_conf_path}; using DummyModel')

    class DummyModel:
        async def generate(self, prompt: str):
            return {"text": "\\section{Introduction}\nThis is an automated test draft.\n\\section{Method}\nDetails omitted for test.\n\\section{Conclusion}\nAuto test complete."}

        async def call(self, prompt: str):
            return {"text": "ok"}

    if model is None:
        print('[paper_generation_agent.py] No model created; using DummyModel')
        model = DummyModel()

    out_dir = os.path.join(iteration_path, 'latex_test')
    agent_conf = {
        'output_dir': out_dir,
        'num_cite_rounds': 3,
    ...
    if paper_conf:
        for key in ('vlm_model', 'big_model', 'small_model', 'template_dir', 'idea_result_dirs'):
            if paper_conf.get(key) is not None:
                agent_conf[key] = paper_conf[key]

    if agent_conf.get('vlm_model') is None:
        print('[paper_generation_agent.py] Warning: No vlm_model found in config; VLM descriptions will be skipped in standalone test')

    agent = PaperGenerationAgent(model, config=agent_conf)
    ...
    for key in ('vlm_model', 'big_model'):
        if agent_conf.get(key) is not None:
            params[key] = agent_conf[key]

    if detected_idea_dirs:
        params['idea_result_dirs'] = detected_idea_dirs

    try:
        res = asyncio.run(agent.execute(context, params))
        print('\n=== Paper Generation Result ===')
        if res.get('latex_dir'):
            print('\nLaTeX directory:', res.get('latex_dir'))
            print('Meta file:', os.path.join(res.get('latex_dir'), 'paper_meta.json'))
    except Exception as e:
